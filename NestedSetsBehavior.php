...
namespace letyii\mongonestedset;

use yii\base\Behavior;
use yii\base\NotSupportedException;
use yii\mongodb\ActiveRecord;; //yii\db\ActiveRecord;
...
//    }
    ...
    protected function buildTreeKeys($items = [], $left = null, $right = null)
    {
        $tree = [];
        if ($left == null)
        ...
        foreach ($items as $key => $item) {
            $childLeft = $item[$this->leftAttribute];
            $childRight = $item[$this->rightAttribute];
            if ($childLeft == $left + 1 && (is_null($right) || $childRight < $right)) {
                $tree[$key] = $this->buildTreeKeys($items, $childLeft, $childRight);
                $left = $childRight;
            }
        }
    ...
    public function buildTreeHtml($keys = [], $items = [])
    {
        if (empty($items)) {
            $items[] = [
                '_id' => $this->owner->getAttribute('_id'),
                'name' => $this->owner->getAttribute('name'),
                'lft' => $this->owner->getAttribute($this->leftAttribute),
                'rgt' => $this->owner->getAttribute($this->rightAttribute),
            ];
            $children = $this->children()->all();
            foreach ($children as $child) {
                $items[] = [
                    '_id' => $child->owner->getAttribute('_id'),
                    'name' => $child->owner->getAttribute('name'),
                    $this->leftAttribute => $child->owner->getAttribute($this->leftAttribute),
                    $this->rightAttribute => $child->owner->getAttribute($this->rightAttribute),
                ];
            }
        ...
        foreach ($keys as $key => $row) {
            if (count($row)) {
                $list.='<li>' . $items[$key]['name'] . $this->buildTreeHtml($row, $items) . '</li>';
            } else {
                $list.='<li>' . $items[$key]['name'] . '</li>';
            }
        }
